...
const writeVisitedFile = (fileName, content) => {
  writeFile(`data/${fileName}__visited.json`, content);
};

const writeCollectedFile = (fileName, content) => {
  writeFile(`data/${fileName}__collected.json`, content);
};

const writeTreeFile = (fileName, content) => {
  writeFile(`data/${fileName}__tree.json`, content);
};

const writeAllFiles = ({fileName, visitedUrls, collectedUrls, tree}) => {
  // Write visited links before searching next
  try {
    writeVisitedFile(fileName, visitedUrls);
  } catch (e) {
  ...
  try {
    writeCollectedFile(fileName, collectedUrls);
  } catch (e) {
