...
const getDataPromises = (data) => {
  ...
  const filePathsList = [filePathTree, filePathVisited];

  const readFilePromises = filePathsList.map(filePath => {
...
const getStaticPromises = () => {
  const filePathCSS = `public/assets/styles.css`;
  const filePathJS = `public/assets/index.js`;
  const filePathsList = [filePathCSS, filePathJS];
...
const showData = async (data, ws) => {
  if(data.listId === 'urls') {
    ...
    const readFilePromises = getDataPromises(data);
...
const downloadData = async (data, ws) => {
  if(data.listId === 'urls') {
    const logsEmit = logs(ws);
    const readFilePromises = getDataPromises(data);
    const staticFilesPromises = getStaticPromises();
    const promisesList = readFilePromises.concat(staticFilesPromises);
    const folderName = getFolderName(data);
