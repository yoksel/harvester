...
const showData = async (data, ws) => {
  if(data.listId === 'urls') {
    ...
    const fileName = `urls--${data.taskId}`;
    const filePathTree = `data/${fileName}__tree.json`;
    const filePathVisited = `data/${fileName}__visited.json`;
    const filePathesList = [filePathTree, filePathVisited];

    const readFilePromises = filePathesList.map(filePath => {
