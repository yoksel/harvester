...
const getUrls = async ({
  ...
  let urls = task.urls;
  const urlsFile = task.urlsFile;

  // Get urls from external file
  if(urlsFile) {
    const urlsFromFile = await readFile(urlsFile)
      .then(result => {
        const data = JSON.parse(result);
        if(data.hrefs) {
          urls = data.hrefs.map(item => item.url);
        }
      })
      .catch(error => {
        logsEmit({
  ...
  if(urls && urls.length > 0) {
    if(urls.length > task.max) {
      urls = urls.slice(0, task.max);
    }
  ...
  const currentUrl = task.startUrl || urls[0];
...
const collectUrls = async ({
  ...
  // There will not be link selector for
  // task collecting data
  if(linksSelectorRestriction) {
  ...
  if (task.dataToCollect) {
    // Get data from page
    dataFromPage = await getDataFromPage({
      page,
      dataToCollect: task.dataToCollect
    });
  }
  ...
  const pageData = await collectPageData({
    ...
    folderName,
    customData: dataFromPage
  });

  if(dataFromPage && Object.keys(dataFromPage).length > 0) {
    Object.keys(dataFromPage).forEach(key => {
      if (!collectedData[key]) {
        // TODO: detect input data type, obj vs array
        collectedData[key] = [];
      }

      collectedData[key] = collectedData[key].concat(dataFromPage[key]);
    });
  }
